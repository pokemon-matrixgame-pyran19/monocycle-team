...
- この対称配置では**窓が常に $(0^\circ,180^\circ)$ の内側に収まる**ため、$\sin\theta$ が $0$ に届く手前で一択に切り替わり、逆数の発散は決して起きず、$|g|\le1$ に保たれます。窓の縁での値は $\cos50^\circ\approx0.64$、窓の底（$\theta=90^\circ$）の値は $\cos30^\circ\cos50^\circ\approx0.56$ になります（$30^\circ,50^\circ$ は各構築の広がり $\gamma_2-\gamma_1=60^\circ,\ \delta_4-\delta_3=100^\circ$ の半分）。

...
ここで $v_1\times v_2>0$ かつ $V_i\parallel +x$ という条件から、おもしろい縛りが出ます。$V_i$ は $v_1-v_2$ と同じ向きなので、これが $+x$ を向くということは、$v_1,v_2$ は $y$ 成分が等しく（同じ高さ $h$）、$v_1$ が右（$x_1>x_2$）。外積を成分で書くと $v_1\times v_2=h(x_1-x_2)$ で、これが正で $x_1>x_2$ だから $h>0$——つまり **$v_1,v_2$ は必ず上半面に乗り、しかも高さは $h=1/R_i$**（$V_i=(x_1-x_2,0)/\{h(x_1-x_2)\}=(1/h,0)$ より）になります。$v_1$ が右側＝小さい偏角なので、偏角を $\gamma_1<\gamma_2\ (\in(0,\pi))$ としておきます。相手も同様で、$\theta=0$ のときの静止系での偏角を $\delta_3<\delta_4\ (\in(0,\pi))$ とすると、回転後は $v_3$ が $\delta_3+\theta$、$v_4$ が $\delta_4+\theta$ を向きます。

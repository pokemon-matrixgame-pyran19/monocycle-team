...
- 相手構築を回すと、2択領域では $g=\dfrac{1}{R_iR_j\sin\theta}$ という $\sin$ の逆数曲線、その外は単体相性 $v_a\times v_b$ に頭打ち。窓の縁（遷移角）は必ず $\sin\theta\ne0$ の側にあるので**発散は配置によらず常に一択化でガードされ**、さらに長さを $1$ に揃えたこの例では一択値が $\sin$ で抑えられて $|g|\le1$ に収まります。窓の中心（$V_i\perp V_j$）が一番安定、という形になりました。

...
- なお、色んなパラメータ用の描画は[こちら](https://pokemon-matrixgame-pyran19.github.io/monocycle-team/tools/rotating-team/experient.html)。
...
- **一択化で頭打ちになる**。数式上二択の数式である$\sin$ の逆数の曲線は分母が０になって無限大になる可能性がある関数(グラフ青線)ですが、実際には必ずその前に単体相性 $\sin$ の値で抑えられ、むしろ関数全体で平坦になっている箇所になっています。
- **二択部分は変化が平ら** 。二択部分は比較的変化が平らな一方、一択になってる箇所で大きく値が変化している＝相性の影響を強く受けることがわかります。二択になってる部分の数式だけから何か言うのは正しくなくて、一択になっている部分含めて総合的に判断することが重要そうです。
- **二択部分は上下対称** 。見た感じ、二択の部分は180°反対側に同じくらいの区間の幅で上下対称な二択が毎回セットで現れます。上と合わせて、どれだけ上に偏ってるか=強い構築かという観点で、二択部分はむしろ寄与しないくらいかもしれません。
...
- 重要なのは、この**頭打ち（一択化）が $\sin\theta$ の発散を必ずガードする**こと。窓の縁（遷移角）は配置によらず必ず $\sin\theta\ne0$ の側に来るので、$\sin\theta$ が $0$ に届いて逆数が暴れ出す前に一択へ切り替わり、$g$ は有限に収まります。
- というわけで実質的に一択になってる箇所含めて総合的に上に偏るように設計することが重要です。

## あとがき

いかがでしたか？ 前回まで「適用外」として放置していた実質一択をちゃんと書き切れて、ついでに相手を回したときのグラフの形まで眺められて満足です。二択のままだと分母の$\sin$ が０になるところが、途中で一択になってガードされてしまって有限値に収まる、というのが個人的にはお気に入りポイントでした。都合の良いところで一択になったり、逆に一択だと大きくなりそうなタイミングで二択に逃がせるように切り替わりの条件を工夫をすれば不利になるθを減らし有利なθを増やすことができる=噛み合いに依らない構築づくりのヒントになりそうな予感がします。

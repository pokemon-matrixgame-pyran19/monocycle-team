...
ゲームの値 $g$ は単体相性ベクトルの外積 $v_a\times v_b$ だけで決まる（[game_value.md](game_value.md)：一択値も2択値 $1/(V_i\times V_j)$ もすべて外積の比）。外積は全ベクトルを同一角だけ回しても不変なので、配置には**全体回転のゲージ自由度**が1つあり、これを使って $V_i$ を $x$ 軸正方向に固定してよい（大きさ $R_i=|V_i|$）。残る本質的な自由度は両構築の**相対的な向き**だけ。

そこで自分を固定したまま、相手構築を剛体回転（$v_3,v_4$ をまとめて同じ角だけ回す）させる。$\theta=0$ では相手も基準姿勢（次節：$v_3,v_4$ が上半面に乗り $V_j\parallel+x$）に置くと約束すると、剛体回転角と $V_j$ の向きが一致し、$V_j$ の向きを角度 $\theta$ として
...
**突き合わせ**：2つの候補集合の共通部分は**常にちょうど1つ**になる（2択も「分岐の1つ」として含む）。「揃い」が出しを固定し（$v_3,v_4$ が同側なら自分の出し、$v_1,v_2$ が同側なら相手の出し）、片側だけまたぎ（$V$ を挟む配置）でも候補の交わりで一意。両側がまたぎのときだけ共通部分が $\{\text{2択}\}$ になり、これが $\theta\bmod\pi\in(\gamma_1,\gamma_2)\cap(\pi-\delta_4,\pi-\delta_3)$（次節の窓）。

...
> なお長さに上限（パワー予算）が無ければ、$\eta_i>\eta_j$ を保ったまま $S_i$ を上げるだけで $\langle g\rangle$ は非有界に増える。意味のある最適化は**長さ予算 $S_i$ 固定**のもとで、そこでは目的は「$\eta_i$ の最大化＝広がり効率を上げる」一点に帰着する（開き角を開く・長さを前向きに非対称化する、の二択）。

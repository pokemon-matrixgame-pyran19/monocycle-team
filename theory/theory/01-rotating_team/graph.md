...
- **2択窓の中心 $\theta=\tfrac{\pi}{2}$ で $g$ が極小**。$\sin\theta$ の逆数は窓の縁に向かって増え、最大の有利は窓の縁（2択と一択の境目）で達成される（「直交が一番安定」）。
- **一択化で頭打ち**。$\sin\theta$ の逆数を伸ばせば跳ね上がる所が、単体相性 $\sin$ の値で抑えられる。
- 対称配置では窓が常に $(0,\pi)$ の内側に収まり、$\sin\theta$ の逆数の発散は起きず $|g|\le1$。
- 片方の構築の広がりが極端に大きいと窓が $0,\pi$ に迫り、そこで初めて $\sin\theta$ の逆数的な大きい値が顔を出す。

...
**$\eta$（広がり効率）を大きく＝不利側の一択ローブを $0$ へ持ち上げ、上下対称を崩す.**
ここは**一択の弧で読む**べきところ。2択域は $\frac{S_iS_j}{4}\frac{K}{\sin\theta}$ が $\theta=\tfrac\pi2,\tfrac{3\pi}2$ を底にする**浅く平らな極値**（窓の縁から底までの落差はわずか）で、$\theta$ に対する振れが一番小さい区間――大きく上下するのは振幅 $\frac{S_iS_j}{4}(1\pm\alpha)(1\pm\alpha)$ で $0$ から頭打ちまで掃く一択の弧のほうで、$\eta$ の効きも平均への寄与もそこに出る。実際、上の式で**$\eta$ は一択の高さには入らず、動かすのは遷移角（弧の位相と境界）だけ**。

決定的な事実：**$\eta_i=\eta_j$ ならグラフは点対称 $g(\theta+\pi)=-g(\theta)$**（有利ローブと不利ローブが完全な鏡像）で一周平均はゼロ。$\eta_i$ を $\eta_j$ より上げるとこの点対称が崩れ、それが平均 $\langle g\rangle=\frac{S_iS_j}{2\pi}(\eta_i-\eta_j)$ の出どころ。崩れ方を $\theta$ 区間で見ると（相手 $\eta_j=0.34$ 固定で $\eta_i:0.34\to0.82$、`tools/rotating-team/`）：

- **主役は不利側（下半面）の一択の弧が $0$ へ持ち上がること**（下半面の一択積分 $-1.95\to-0.38$）。相手を「自分の苦手な向き」へ回されても、広いチームは最悪の単体相性がマシになり**負けが浅くなる**。遷移角が動いて不利な弧の占める区間が痩せ・位相がずれる結果で、頭打ち高さ自体は変えていない。
- 有利側（上半面）の一択も伸びる（$+1.95\to+2.29$）が、変化量は不利側の持ち上がりの $1/4$ 程度。$\eta$ で勝つのは**好相性をさらに伸ばす**より**苦手を埋める**効きが大きい。
- **2択域はむしろ痩せて、平均には効かない.** 床の因子 $\sqrt{(\eta^2-\alpha^2)(1-\eta^2)}/\eta$ は $\eta\to1$ で $0$、上半面の2択の取り分は $0.63\to0.38$ と**減る**。しかも2択部分は厳密に点対称 $g(\theta+\pi)=-g(\theta)$ のまま（窓は $\pi$ 反対側にペア、[rotation_average.md](rotation_average.md)）で、上の正の窓を高くすれば下の負の窓が同じだけ深くなり**必ず相殺**する。だから「$\eta$ を上げて2択の取り分を稼ぐ」は誤り――広げる効果は2択を**薄くしながら**、一択ローブの上下非対称として現れる。

符号と増幅は平均の式どおり：$\eta_i>\eta_j$ で一周ならして有利、勝ち幅は $S_iS_j$ 倍。なお幅 $\Delta$（$\tan\tfrac\Delta2=\sqrt{\tfrac{\eta^2-\alpha^2}{1-\eta^2}}$）が広がってP1→P2→P3 と窓は開いていくが、それは「読み合いの起きる $\theta$ が増える」話で、上の通り平均の損得とは別物。
